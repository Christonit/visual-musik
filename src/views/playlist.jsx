...
import {close_video_prev, set_youtube_list, clear_youtube_list} from "../actions";
...
class Playlist extends Component {
    ...
    searchForVideo(query) {
        YoutubeSearchForVideos(query).then(data => {
            ...
            console.log(yt_response);
            // let yt_response = data;
                ...
                {active_playlist != null && (
                    <div className="container">
                        <div className="playlist-header">
                            <img className="music-image-medium" src={active_playlist.images[0].url} alt="" />
                            <div className="ml--20">
                        ...
                        <div className="playlist-tracks">
                            ...
                            {tracks.map((song, key) => {
                                ...
                                return (
                                    <div className="playlist-tracks-row">
                                        <span>{key + 1}</span>
                                        <span>
                                            <p className="text bold">{truncateText(song.track.name, 44)}</p>
                                            <p className="text label">
                                                {artists.map((item, key) =>
                                                    key == artists_length - 1 ? `${item.name}` : `${item.name}, `,
                                                )}
                                            </p>
                                        </span>
                                        <span className="text label">
                                            {duration.min} {duration.min >= 10 ? "mins" : "min"} {duration.sec} sec
                                        </span>
                                        <span className="mr--16">
                                            <button
                                                className="button"
                                                onClick={() => this.searchForVideo(search_param)}>
                                                Search
                                            </button>
                                        </span>
                                    </div>
